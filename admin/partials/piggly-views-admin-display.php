...
<div class="piggly-views">
    <div class="wrapper">
        ...
        <h1><?php _e( 'About', PIGGLY_VIEWS_NAME ) ?></h1>

        <p><?php _e('Hello there! The Piggly View plugin is quite simple to use.', PIGGLY_VIEWS_NAME );?></p>

        <ol>
            <li><?php _e('It will store all views for a Single Post. Below you can configure it to ignore views to logged users  and disable for some posts.', PIGGLY_VIEWS_NAME ); ?></li>
...
&lt;/a&gt;
        ...
        <form method="POST">
            ...
            <input type="checkbox" name="<?=PIGGLY_VIEWS_NAME.'_show_column'?>" id="<?=PIGGLY_VIEWS_NAME.'_show_column'?>" value="1" <?php if ( $current_settings['show_column'] ) : echo 'checked'; endif; ?>>
            <label for="<?=PIGGLY_VIEWS_NAME.'_show_column'?>"><?php _e( 'Display View Column', PIGGLY_VIEWS_NAME ); ?></label>
            <br/><br/>
            <input type="checkbox" name="<?=PIGGLY_VIEWS_NAME.'_disable_admin'?>" id="<?=PIGGLY_VIEWS_NAME.'_disable_admin'?>" value="1" <?php if ( $current_settings['disable_admin'] ) : echo 'checked'; endif; ?>>
            <label for="<?=PIGGLY_VIEWS_NAME.'_disabled_admin'?>"><?php _e( 'Disable tracking for Logged Users', PIGGLY_VIEWS_NAME ); ?></label>
            <br/><br/>
            <input type="checkbox" name="<?=PIGGLY_VIEWS_NAME.'_format'?>" id="<?=PIGGLY_VIEWS_NAME.'_format'?>" value="1" <?php if ( $current_settings['format'] ) : echo 'checked'; endif; ?>>
            <label for="<?=PIGGLY_VIEWS_NAME.'_format'?>"><?php _e( 'Convert Views from 1000 to 1K format', PIGGLY_VIEWS_NAME ); ?></label>
            <br/><br/>
            <input type="checkbox" name="<?=PIGGLY_VIEWS_NAME.'_cache'?>" id="<?=PIGGLY_VIEWS_NAME.'_cache'?>" value="1" <?php if ( $current_settings['cache'] ) : echo 'checked'; endif; ?>>
            <label for="<?=PIGGLY_VIEWS_NAME.'_cache'?>"><?php _e( 'Cache Views Data', PIGGLY_VIEWS_NAME ); ?></label>
            
            <div class="divisor"></div>
            
            <input type="checkbox" name="<?=PIGGLY_VIEWS_NAME.'_keep_options'?>" id="<?=PIGGLY_VIEWS_NAME.'_keep_options'?>" value="1" <?php if ( $current_settings['keep_options'] ) : echo 'checked'; endif; ?>>
            <label for="<?=PIGGLY_VIEWS_NAME.'_keep_options'?>"><?php _e( 'Keep options when uninstall the plugin', PIGGLY_VIEWS_NAME ); ?></label>
            <br/><br/>
            
            <input type="checkbox" name="<?=PIGGLY_VIEWS_NAME.'_keep_db'?>" id="<?=PIGGLY_VIEWS_NAME.'_keep_db'?>" value="1" <?php if ( $current_settings['keep_db'] ) : echo 'checked'; endif; ?>>
            <label for="<?=PIGGLY_VIEWS_NAME.'_keep_db'?>"><?php _e( 'Keep database when uninstall the plugin', PIGGLY_VIEWS_NAME ); ?></label>
            
            <div class="divisor"></div>
            
            <label for="<?=PIGGLY_VIEWS_NAME.'_cache_hour'?>"><?php _e( 'Flush cache after X hours', PIGGLY_VIEWS_NAME ); ?></label>
            <br/>
            <input class="input" type="number" name="<?=PIGGLY_VIEWS_NAME.'_cache_hour'?>" id="<?=PIGGLY_VIEWS_NAME.'_cache_hour'?>" value="<?=$current_settings['cache_hour'];?>">
            <br/><br/>
    ...
    <?php if ( $updated ) : ?>
        <div class="message success"><?php _e( 'Your settings were saved...', PIGGLY_VIEWS_NAME); ?></div>
    <?php endif; ?>
